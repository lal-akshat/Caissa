...
public class Search
{
    private readonly PossibleMoves _moveGenerator = new PossibleMoves();

    public Move? FindBestMove(Board board, PieceColor color, int depth)
    {
        List<Move> moves = _moveGenerator.GenerateMoves(board, color);
        if (moves.Count == 0)
            return null;
        ...
        {
            board.MakeMove(move);
            int score = Minimax(board, depth - 1, color.Opposite(), int.MinValue, int.MaxValue);
            board.UndoMove(move);
    ...
    private int Minimax(Board board, int depth, PieceColor color, int alpha, int beta)
    {
        if (depth == 0)
        ...
        if (color == PieceColor.White)
        {
            ...
            {
                board.MakeMove(move);
                int score = Minimax(board, depth - 1, color.Opposite(), alpha, beta);
                board.UndoMove(move);
    ...
    private int Quiescence(Board board, PieceColor color, int alpha, int beta)
    {
        ...
        {
            board.MakeMove(move);
            int score = Quiescence(board, color.Opposite(), alpha, beta);
            board.UndoMove(move);
        ...
        return color == PieceColor.White ? alpha : beta;
    }
    ...
    private int GetMoveScore(Move move)
    {
        ...
        {
            score += 1000;
            score += Evaluation.GetPieceValue(move.CapturedPiece.Type);
        }
